...
- **防误触顺延逻辑**：点击“已保活/续期”按钮时，会在**原截止日期基础上自动顺延指定的周期天数**，并带有确认弹窗，绝不覆盖长期续费记录。
...
2. 在左侧菜单找到 **Storage & Databases** -> **KV**。
    
3. 点击 **创建命名空间 (Create Namespace)**，名称填写 `DOMAIN_KV` 并保存。
    

### 第二步：创建并部署 Worker

1. 在左侧菜单点击 **Workers 和 Pages (Workers & Pages)** -> **创建应用程序 (Create Application)** -> **创建 Worker**。
    
2. 输入应用名称（如 `domain-keeper`），点击 **部署 (Deploy)**。
    
3. 进入编辑界面，点击 **编辑代码 (Edit code)**。
    
4. 将本仓库中的 `worker.js` 代码完整复制粘贴进去，覆盖原有的示例代码。
...
1. 返回该 Worker 的管理后台，切换到 **设置 (Settings)** 标签页 -> 点击 **Variables (变量)**。
    
2. 向下滚动到 **KV Namespace Bindings (KV 命名空间绑定)**，点击 **Add binding (添加绑定)**。
...
- **如何修改默认密码**： 在 Cloudflare Worker 的 **Settings** -> **Variables** -> **环境变量 (Environment Variables)** 中添加变量 `AUTH_SECRET`，值为你的自定义密码。
